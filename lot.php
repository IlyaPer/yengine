...
<?php
    ...
    if ($con == false) {
      ...
      $result_bets = mysqli_fetch_all($result_bet, MYSQLI_ASSOC);
      if($result_bets = 0){
        $how_many_bets = 0;
      } else{
      $how_many_bets = mysqli_num_rows($result_bets); //Считаю кол-во ставок
    }
      $sql_bet_id = "SELECT bets.id FROM bets JOIN lots ON lots.id = bets.lot_id WHERE lots.id = $id;";

      $bet_id = 1;
      //Подсчет даты создания ставок
      $sql_when_bet = "SELECT NOW() - time_bet FROM bets WHERE id = $bet_id;";
      $result_when = mysqli_query($con, $sql_when_bet);
      $when = mysqli_fetch_all($result_when, MYSQLI_ASSOC);
          ...
          <div class="history">
            ...
            <table class="history__list">
              <tr class="history__item">
                ...
                <td class="history__price"><?= htmlspecialchars($lot['bet_step']); ?></td>
                <td class="history__time"><?= $when['SELECT NOW() - time_bet']; ?></td>
              </tr>
            </table>
          </div>
        </div>
